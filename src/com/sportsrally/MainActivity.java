...
public class MainActivity extends Activity implements LocationListener {
	...
	protected void onStart() {
		...
		mgr = (LocationManager) getSystemService(LOCATION_SERVICE);
		bestProvider = null;
		if(isNetworkEnable && !isGpsEnabled) { bestProvider = LocationManager.NETWORK_PROVIDER;
		lastKnownLocation = mgr.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);}
		if(isGpsEnabled) {bestProvider = LocationManager.GPS_PROVIDER;
		lastKnownLocation = mgr.getLastKnownLocation(LocationManager.GPS_PROVIDER);}
		if(bestProvider == null) bestProvider = LocationManager.PASSIVE_PROVIDER;
		if (lastKnownLocation == null) {
			lastKnownLocation = mgr.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);}
		mgr.requestLocationUpdates(MainActivity.bestProvider, MIN_TIME, MIN_DIST, this);
		if(bestProvider != LocationManager.PASSIVE_PROVIDER){
		createCancelProgressDialog("系統", "取得"+bestProvider+"定位中..請稍待", "取消");}else
		{    btnToggle.setVisibility(View.INVISIBLE);
	         ...
	         dialog.setPositiveButton("更改設定", new DialogInterface.OnClickListener() {  
	...
	public void onLocationChanged(Location location) {
		...
		gmap.animateCamera(CameraUpdateFactory.newLatLngZoom(myapp.nowPoint, zoom));
		if(marker!=null) marker.remove();
		marker = gmap.addMarker (new MarkerOptions()
						.position(myapp.nowPoint)
				.icon(BitmapDescriptorFactory.fromResource(R.drawable.icon))
					);
		
  		if(TimerService.mState.equals(TimerService.State.Running)&&myapp.lastPoint!=null&&myapp.nowPoint!=null){
...
//		mgr.requestLocationUpdates(MainActivity.bestProvider, MIN_TIME, MIN_DIST, this);
    ...
    void checkProvider(){
		...
		isNetworkEnable = mgr.isProviderEnabled(LocationManager.NETWORK_PROVIDER); 
    ...
    void showNetworkSetup(){
         ...
         dialog.setNeutralButton("網路", new DialogInterface.OnClickListener() {
        	    public void onClick(DialogInterface dialog, int which) {
        	     // TODO Auto-generated method stub
